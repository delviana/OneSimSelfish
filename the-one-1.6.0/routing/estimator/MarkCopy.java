 ...
 */
public class MarkCopy implements RoutingDecisionEngineImproved, MarkingNode, Status {
    ...
    public MarkCopy(MarkCopy prototype) {
    ...
    public void connectionUp(DTNHost thisHost, DTNHost peer) {
        MarkCopy partner = getOtherEstimatorRouter(peer);
...
//    }
    ...
    private MarkCopy getOtherEstimatorRouter(DTNHost host) {
        MessageRouter otherRouter = host.getRouter();
        assert otherRouter instanceof DecisionEngineRouterImproved : "This router only works "
                + " with other routers of same type";

        return (MarkCopy) ((DecisionEngineRouterImproved) otherRouter).getDecisionEngine();
    }

    @Override
    public RoutingDecisionEngine replicate() {
        return new MarkCopy(this);
    }

    private void recapture() {
        Set<DTNHost> newComerHosts = new HashSet<>();
        DTNHost thisHost = null;
        if (!recapturedNode.contains(markCount)) {
            recapturedNode.add(thisHost);
            newComerHosts.add(thisHost);
        }
