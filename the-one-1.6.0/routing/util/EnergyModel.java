...
public class EnergyModel implements ModuleCommunicationListener {
    ...
    /**
     * Initial units of energy -setting id ({@value}). Can be either a single
     * value, or a range of two values. In the latter case, the used value is a
     * uniformly distributed random value between the two values.
     */
    ...
    /**
     * Sets the current energy level into the given range using uniform random
     * distribution.
     *
     * @param range The min and max values of the range, or if only one value is
     * given, that is used as the energy level
     */
     ...
     * If the energy level goes below zero, sets the level to zero. Does nothing
     * if the warmup time has not passed.
     *
    ...
    protected void reduceEnergy(double amount) {
        ...
        if (comBus == null) {
            return;
            /* model not initialized (via update) yet */
        }
    ...
    /**
     * Reduces the energy reserve for the amount that is used when another host
     * connects (does device discovery)
     */
